...
<div
  className={`${styles.gnb} ${
    selectMenuType === styles.fullDown && fullDownState ? styles.active : ""
  }`}
...
>
  <div className={styles.depth01}>
    <ul>
      {sortedItems
        .filter((itemDepth1) => itemDepth1.depth === 1)
        .map((itemDepth1, index) => {
          const hasDepth2Items = sortedItems.some(
            (itemDepth2) =>
              itemDepth2.menuId === itemDepth1.menuId && itemDepth2.depth === 2
          );
          const hasDepth3Items = sortedItems.some(
            (itemDepth3) =>
              itemDepth3.menuId === itemDepth1.menuId && itemDepth3.depth === 3
          );
                ...
                <div className={styles.depth02}>
                  ...
                  <ul className={styles.group}>
                    {sortedItems
                      .filter(
                        (itemDepth2) =>
                          itemDepth2.menuId === itemDepth1.menuId &&
                          itemDepth2.depth === 2
                      )
                      .map((itemDepth2) => (
                        <li
                          key={itemDepth2.id}
                          className={
                            selectDepth2Id === itemDepth2.id ? styles.active : ""
                          }
                            ...
                            <div className={styles.depth03}>
                              <ul className={styles.group}>
                                {sortedItems
                                  .filter(
                                    (itemDepth3) =>
                                      itemDepth3.menuId === itemDepth1.menuId &&
                                      itemDepth3.depth === 3
                                  )
                                  .map((itemDepth3) => (
                                    <li key={itemDepth3.id}>
